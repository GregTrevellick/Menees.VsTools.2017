namespace Menees.VsTools.Tasks
{
	...
	internal sealed class HierarchyVisitor
	{
		...
		private Stack<HierarchyItem> ancestors = new Stack<HierarchyItem>();
		private List<HierarchyItem> items = new List<HierarchyItem>();
		private HashSet<string> visitedProjects = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
		...
		private sealed class HierarchyNode
		{
			...
			public HierarchyNode(IVsHierarchy hierarchy, uint itemId)
			{
				...
				int hr = this.Hierarchy.GetCanonicalName(this.ItemId, out canonicalName);
				if (ErrorHandler.Succeeded(hr))
				{
			...
			public bool TryGetNestedHierarchy(out HierarchyNode nestedNode)
			{
				...
				uint nestedItemId;
				int hr = this.Hierarchy.GetNestedHierarchy(this.ItemId, ref hierGuid, out nestedHierarchyObj, out nestedItemId);
				if (hr == VSConstants.S_OK && nestedHierarchyObj != IntPtr.Zero)
				{
					nestedHierarchy = Marshal.GetObjectForIUnknown(nestedHierarchyObj) as IVsHierarchy;
			...
			public TResult GetProperty<TPropId, TResult>(TPropId propertyId, TResult defaultValue = default(TResult))
				where TPropId : struct
			{
				...
				where TPropId : struct
			{
				...
				{
					Guid guid;
					hr = hierarchy.GetGuidProperty(itemId, propId, out guid);
					result = hr == VSConstants.S_OK ? (TResult)(object)guid : defaultValue;
				}
				else
				{
					object value;
					hr = hierarchy.GetProperty(itemId, propId, out value);
