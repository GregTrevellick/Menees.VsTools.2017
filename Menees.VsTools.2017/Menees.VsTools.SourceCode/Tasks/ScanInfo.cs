namespace Menees.VsTools.Tasks
{
	...
	internal sealed class ScanInfo
	{
		...
		private HashSet<Delimiter> delimiters = new HashSet<Delimiter>();
		...
		private sealed class Cache
		{
			...
			private ConcurrentDictionary<string, ScanInfo> extensions = new ConcurrentDictionary<string, ScanInfo>(StringComparer.OrdinalIgnoreCase);
			private Dictionary<Language, ScanInfo> languages = new Dictionary<Language, ScanInfo>();
			...
			{
				ScanInfo result;
				if (!this.TryGet(language, out result))
				{
		...
		private sealed class Delimiter
		{
			...
			{
				Delimiter delimiter = obj as Delimiter;
				bool result = delimiter == null ? false : StringComparer.OrdinalIgnoreCase.Equals(this.id, delimiter.id);
				return result;
			}
