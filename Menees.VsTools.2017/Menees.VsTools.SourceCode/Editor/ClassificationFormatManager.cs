namespace Menees.VsTools.Editor
{
	...
	internal sealed class ClassificationFormatManager : IVsTextManagerEvents
	{
		...
		private Dictionary<string, Guid> formatNameToCategoryMap = new Dictionary<string, Guid>();
		private IVsFontAndColorStorage storage;
		private IVsFontAndColorStorage2 storage2;
		...
		{
			// Profiling showed we spent almost 5% of our startup time waiting to enumerate categories here.
			// So we'll do that work on a background thread with a fire-and-forget task.
			Task.Run(() =>
			{
		...
		private IDisposable OpenCategory(Guid category, __FCSTORAGEFLAGS flags)
		{
			...
			if (ErrorHandler.Succeeded(hr))
			{
				...
				result = new Disposer(() => ErrorHandler.Succeeded(this.storage.CloseCategory()));
			}
